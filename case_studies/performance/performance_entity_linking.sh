...
START=$(date +%s.%N)
python3 ~/KGExtractionToolbox/src/kgextractiontoolbox/entitylinking/vocab_entity_linking.py $PUBMED_SAMPLE -c pubmed_benchmark -v $PHARMACY_VOCAB --skip-load -f --workers 32 2>> /dev/null 1>>/dev/null
END=$(date +%s.%N)
